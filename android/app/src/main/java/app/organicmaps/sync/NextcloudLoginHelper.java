...
public class NextcloudLoginHelper
{
        ...
        ThreadPool.getWorker().execute(() -> {
          try
          {
            ...
            if (responseCode / 100 != 2)
              throw new Exception(context.getString(responseCode == 404 ? R.string.make_sure_url_is_nextcloud : R.string.error_connecting_to_server));
  ...
  @MainThread
  private static void loginFlowStepTwo(Context context, LoginParams loginParams, SyncBackend.LoginSuccessCallback loginSuccessCallback)
  {
      ...
      CustomTabsServiceConnection connection = new CustomTabsServiceConnection()
      {
        ...
        public void onCustomTabsServiceConnected(@NonNull ComponentName name, CustomTabsClient client)
        {
              ...
              if (navigationEvent == CustomTabsCallback.NAVIGATION_FINISHED)
              {
                ...
                ThreadPool.getWorker().execute(() -> {
                  try
                  {
                    ...
                    SyncPrefs.AddAccountResult result = SyncPrefs.getInstance(context).addAccount(BackendType.Nextcloud, authState);
                    if (context instanceof Activity)
                      context.startActivity(new Intent(context, context.getClass()).addFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT));
                    mLoginComplete = true;
                    switch (result)
                    {
                      case Success ->
          ...
          CustomTabsIntent customTabsIntent = new CustomTabsIntent.Builder(session)
              .setInitialActivityHeightPx(UiUtils.getDisplayTotalHeight(context) * 9 / 10) // Set initial custom tabs height to 90% of total display height
              .build();
