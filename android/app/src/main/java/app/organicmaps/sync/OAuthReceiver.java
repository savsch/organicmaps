...
public class OAuthReceiver extends BroadcastReceiver
{
  ...
  private void googleAuth(Context appContext, final AuthorizationResponse response)
  {
    ...
    service.performTokenRequest(response.createTokenExchangeRequest(), (tokenResponse, exception) -> {
      ...
      String idToken = tokenResponse.idToken; // The "email" (or a similar) scope is necessary for the response to have an id_token
      try
      {
        // the idToken is a JWT with the payload being of the form: { ..., "email": string, ...}
        String payload = new String(Base64.decode(Objects.requireNonNull(idToken).split("\\.")[1], Base64.DEFAULT), StandardCharsets.UTF_8);
        ...
        SyncPrefs.AddAccountResult result = SyncPrefs.getInstance(appContext).addAccount(BackendType.GoogleDrive, authState);
